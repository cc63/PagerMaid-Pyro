...
async def rate(message: Message):
    ...
    def is_currency(value):
        return len(value) == 3 and value.isalpha() and value.upper() in rate_data.currencies
    ...
    if len(message.parameter) == 1:
        ...
        if is_currency(to_):
            ...
            from_ = message.parameter[0].upper().strip()
            to_ = "CNY"
            amount_expr = message.parameter[1].strip()
            try:
                cmd = f'echo "scale=4;{amount_expr}" | bc'
                result = await execute(cmd)
                amount = float(result.strip())
            except Exception as e:
                await message.edit(f"计算错误: {e}")
                return
    elif len(message.parameter) == 3:
        from_ = message.parameter[0].upper().strip()
        to_ = message.parameter[1].upper().strip()
        amount_expr = message.parameter[2].strip()
        if not is_currency(to_):
            await message.edit(
    ...
    try:
        text = await rate_data.get_rate(from_, to_, amount)
    except Exception as e:
        text = str(e)
